...
void receiveEvent(int bytes) {
    if (Wire.available() >= 3) {  // Pastikan ada 3 byte data masuk
        ...
        // **Gerakan Maju**
        if (throttle > 10) { // angka 10 dipilih sebagai batas aman agar sistem tidak mudah bereaksi terhadap noise kecil dari input.
            setBLDC(map(throttle, 0, 100, ESC_IDLE, ESC_MAX));
        }
        // **Gerakan Mundur (Putar Balik)**
        else if (throttle < -10) {
            // Putar ke kanan dulu untuk mundur
        ...
        if (turn > 10) {  // Belok kanan
            servoRight.write(map(turn, 0, 100, 90, 120));
            servoLeft.write(map(turn, 0, 100, 90, 150));
        } else if (turn < -10) {  // Belok kiri
            servoRight.write(map(turn, -100, 0, 60, 90));
            servoLeft.write(map(turn, -100, 0, 30, 90));
        } else {  // Netral
            servoRight.write(servoNeutralRight);
            servoLeft.write(servoNeutralLeft);
        }
