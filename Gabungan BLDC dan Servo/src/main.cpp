...
// Variabel kontrol
const int servoNeutralRight = 90;  // Posisi netral servo kanan
const int servoNeutralLeft = 90;   // Posisi netral servo kiri

// Nilai analog joystick
int throttle = 0;  // Kontrol maju-mundur (y-axis joystick)
int vertical = 0;  // Kontrol naik-turun (z-axis joystick)
int turn = 0;      // Kontrol belok kanan-kiri (x-axis joystick)
...
void receiveEvent(int bytes) {
    if (Wire.available() >= 3) {  // Pastikan ada 3 byte data masuk
        ...
        turn = Wire.read();       // Belok (-100 sampai 100)
        ...
        if (vertical > 10) {
            ...
            servoRight.write(map(vertical, 0, 100, servoNeutralRight, 135));
            servoLeft.write(map(vertical, 0, 100, servoNeutralLeft, 135));
        } else if (vertical < -10) {
            esc_vertical.writeMicroseconds(map(vertical, -100, 0, ESC_MIN, ESC_IDLE));
            servoRight.write(map(vertical, -100, 0, 45, servoNeutralRight));
            servoLeft.write(map(vertical, -100, 0, 45, servoNeutralLeft));
        } else {
